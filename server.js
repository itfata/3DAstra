...
const { createCrmItem } = require("./services/crm");
...
function validateLeadPayload(payload, file) {
  if (!file) {
    return "Додайте файл моделі або архів із моделлю.";
  }
  ...
  if (!allowedMaterials.has(payload.material)) {
    return "Обрано некоректний матеріал.";
  }
...
app.get("/health", (_req, res) => {
  res.json({ ok: true, maxFileSizeMb });
});
